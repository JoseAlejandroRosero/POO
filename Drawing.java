...
public class Drawing {
	...
	public Drawing(LinkedList<Figure> figures) {
	...
	public LinkedList<Figure> getSelected() {
		LinkedList<Figure> nl = new LinkedList<Figure>();
	...
	public void load(ObjectInputStream ois, String filename) throws ClassNotFoundException, IOException {
		figures = (LinkedList<Figure>) ois.readObject();
	...
	public ControlPoint getControlPoint(Point p) {
		for(Iterator<Figure> it = figures.descendingIterator();it.hasNext();){
			Figure f=it.next();
			if(f.contains(p))return f.getControlPoints(p);
		}
		return null;
	}
